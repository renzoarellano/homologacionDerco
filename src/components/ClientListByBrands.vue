            ...
            <b-col cols="12">
              <b-row>
                <b-col cols="12" class="head">
                  <h4>:: Clientes</h4>
                  <b-btn @click.stop="descargar_csv">Descargar Lista</b-btn>
                </b-col>
              </b-row>
              ...
              <b-row>
                <b-col cols="12" class="breadcrumb">
                  <b-link :to="{ name: 'BrandListClient' }">Clientes por marcas</b-link> &raquo; Lista de clientes {{ this.$route.params.brand }}:
                </b-col>
              </b-row>
              <b-row>
...
export default {
  ...
  data() {
    return {
      fields: {
        first_name: { label: "Nombre", sortable: true, class: "text-left" },
        last_name: { label: "Apellidos", sortable: true, class: "text-center" },
        brand_w2l: { label: "Marca", sortable: false, class: "text-center" },
        model_w2l: { label: "Modelo", sortable: true, class: "text-center" },
        local_w2l: { label: "Local", sortable: true, class: "text-center" },
  ...
  methods: {
    ...
    search(){
      ...
      axios.get(`/clients-brands/search/`+this.$route.params.brand+'/'+this.dates.date1+'/'+this.dates.date2+'/'+this.$route.params.page)
      .then(response => {
    ...
    descargar_csv(){
      axios.defaults.headers.common['Authorization'] = localStorage.getItem('jwtToken')
      axios.get(`/client/excel/`+this.$route.params.brand+'/'+this.dates.date1+'/'+this.dates.date2)
      .then(response => {
    ...
    linkGen (pageNum) {
      return {
        path: '/clients-by-brands/' + this.$route.params.brand + '/page/' + pageNum
      }
    },
    apply(){

            ...
            <b-col cols="12">
              ...
              <b-table striped hover :items="brands" :fields="fields">
                <template slot="actions" slot-scope="row">
                  <b-btn size="sm" @click.stop="details(row.item)">Ver oficinas</b-btn>
                </template>
              </b-table>
...
export default {
  ...
  created () {
    document.title = 'Administración Derco - Oficinas por marcas'
  ...
  methods: {
    details (brand) {
      this.$router.push({
        ...
        params: { slug: brand.slug, brand: brand.name }
      })
    },
    logout () {
